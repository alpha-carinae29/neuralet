...
class WebGUI:
    ...
    def __init__(self, config, engine_instance):
        ...
        self._fourcc_vid = cv.VideoWriter_fourcc(*'XVID') 
        resolution = [int(i) for i in self.config.get_section_dict('App')['Resolution'].split(',')]
        out_width = resolution[0]
        out_height = resolution[1]
        self._output_cap_vid = cv.VideoWriter("/repo/applications/smart-distancing/data/out.avi",self._fourcc_vid, 20.0, (out_width, out_height))
